...
export const exhibits = [
  { id: "dropdown", name: "The Radio Text Receiver", category: "Forms", color: "pink", tagline: "Your message is somewhere in the static.", description: "Tune into one letter at a time. Reception must be painfully exact.", lesson: "The keyboard was receiving too many letters at once. Each character now broadcasts on its own inconvenient frequency.", fix: "The antenna was unplugged. A text box now receives entire sentences.", worseChange: "Every character sends the stations somewhere new.", preview: `<div class="thumb-scene thumb-radio"><span class="thumb-kicker">NOW RECEIVING: ONE LETTER</span><div class="thumb-radio-face"><div class="thumb-radio-readout"><strong>H</strong><div><b>2496 Hz</b><span>TEXT / MONO</span></div></div><div class="thumb-radio-scale"><i></i></div><div class="thumb-radio-message">MESSAGE: H<span>_</span></div></div><small class="thumb-footer">One hertz off. Nothing but static.</small></div>`, render: renderRadioText },
...
function renderPasswordGym({ stage, mode }) {
  ...
  shell("STRENGTH TRAINING FOR A COMPLETELY FAKE PASSWORD", fixed ? "A passphrase, without the obstacle course." : "Your password needs more reps.",
    `Invent a throwaway phrase. NEVER enter a real password.${fixed ? " There is one requirement, shown upfront and checked as you type." : ` Meet each visible rule to reveal the next.${worse ? " Hard mode keeps a final surprise in reserve." : " The challenge is long but solvable."}`} This is a design puzzle, not security advice or a password-strength test.`,
...
function renderCatCaptcha({ stage, mode }) {
  ...
  shell("CAT-PCHA / RODENT-BASED AUTHENTICATION", "Prove you're human. Become a mouse.",
    `Collect all ${cheeseLocations.length} cheeses, then reach the mouse hole. The cat moves after you.${worse ? " Hard mode has a few house rules; watch the dashboard closely." : ""} Getting caught means starting over. The chase is turn-based, so take your time. This is a pretend CAPTCHA, not a real security check.`,
...
function renderExpandingForm({ stage, mode }) {
  ...
  shell("GROWTH IS OUR ONLY METRIC", fixed ? "A form that respects personal space." : "Your answer has expanded our horizons.",
    fixed ? "Fill in the four demo fields. Their spacing stays put, and nothing is sent or saved." : `Fill in four demo fields. Each answer sends the next one farther away, and Message has especially big plans.${worse ? " Hard mode also folds under long answers." : ""} Scroll inside the form. Compress form preserves your fictional answers; the next edit expands it again.`,
...
function renderCorrectingSearch({ stage, mode }) {
  ...
  shell("YOUR INTENT, REVISED BY COMMITTEE", fixed ? "Search exactly what you typed." : "Did you mean something completely different?",
    fixed ? "Try quiet cafes, weather, museum hours, accessible forms, or cat pictures. Results are fictional and local; nothing is sent to a search service." : `Type a search and wait for a very confident correction. Reject its suggestions to reclaim your original query.${worse ? " Each appeal needs a short explanation." : ""} Results are fictional and local; nothing leaves this page.`,
